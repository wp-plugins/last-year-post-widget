...
// Icon files.
define('PLUS', get_settings('siteurl') . '/wp-content/plugins/last-year-post-widget/icon_plus.gif');
define('MINUS', get_settings('siteurl') . '/wp-content/plugins/last-year-post-widget/icon_minus.gif');

function last_year_post_widget_init()
{
	...
	function last_widget($args)
	{
		...
		// Author credit...
		if($options['credit'] == "on") echo '<span class="last_year_credit"><a href="http://wordpress.org/extend/plugins/last-year-post-widget/">Last Year Post Widget</a></span>';
		echo $after_widget;
	}
	...
	unregister_sidebar_widget('Last Year Post');
	register_sidebar_widget('Last Year Post', last_widget);
	register_widget_control('Last Year Post', 'last_widget_control', 200, 90);
}
...
add_action('plugins_loaded', 'last_year_post_widget_init');
?>